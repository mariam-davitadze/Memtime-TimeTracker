...
import { Badge, Button, Card, Layout, Space, Typography, Row, Col, Segmented } from "antd";
...
const statusColor = (status: string) =>
  status === "completed" ? "#95d5b2" : status === "in-progress" ? "#ffd580" : "#d9d9d9";
        ...
        {filteredClients.map((client) => (
          <Col xs={24} sm={12} md={8} lg={6} key={client.id}>
            <Badge.Ribbon text={client.status} color={statusColor(client.status)}>
              ...
              >
                <Title level={4} style={{ margin: 0 }}>{client.name}</Title>
